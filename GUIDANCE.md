...
Codegen generates `emitOnValueChanged:` onto an abstract base CLASS named `NativeMathSpecBase`, not onto the `<NativeMathSpec>` protocol your class currently adopts. This is the one place in the exercise where you modify a file from an earlier step.

First, change the class declaration in `ios/NativeModulesTraining/RCTNativeMath.h` to inherit from the codegen-generated base class:
...
The class still conforms to the protocol; it now also inherits the `_eventEmitterCallback` ivar and the `emitOnValueChanged:` method from the base class.

Then in `ios/NativeModulesTraining/RCTNativeMath.mm`, call the generated emitter from inside `add`:
...
The argument type matches what you declared in the TS spec (`number` becomes `NSNumber *` here). Without the parent class change in the `.h` file, this line fails to compile with `no visible @interface for 'RCTNativeMath' declares the selector 'emitOnValueChanged:'`.
